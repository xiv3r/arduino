...
// =============================================================================
#define MQTT_CLIENT_ID_PREFIX      "esp32_16ch_"
#define MQTT_KEEPALIVE             60
#define MQTT_RECONNECT_INITIAL     2000
#define MQTT_RECONNECT_MAX         60000
#define MQTT_RECONNECT_MULTIPLIER  2
#define MQTT_STATUS_INTERVAL       60000UL
static const unsigned long MQTT_RECONNECT_CHECK = 10000UL;
...
unsigned long mqttReconnectDelay = MQTT_RECONNECT_INITIAL;
...
private:
    ...
    unsigned long lastMQTTReconfigure = 0;
};
...
void handleMQTTCommand(uint8_t relay, const char* payload);
void handleMQTTAllCommand(const char* payload);
void setupMQTTWill();
void handleMQTTStatusPublish();
String getRelayStateTopic(uint8_t index);
String getRelayCommandTopic(uint8_t index);
String getAllCommandTopic();
...
// =============================================================================
String getRelayStateTopic(uint8_t index) {
    String topic = String(mqttConfig.topic_prefix);
    topic += "/relay/";
    topic += String(index);
    return topic;
}
...
String getRelayCommandTopic(uint8_t index) {
    return getRelayStateTopic(index) + "/set";
}
...
void onMQTTConnect(bool sessionPresent) {
    mqttConnected = true;
    mqttReconnectDelay = MQTT_RECONNECT_INITIAL;
    health.mqttFailures = 0;
    
    // Subscribe to command topics for all configured relays
    for (uint8_t i = 0; i < gpioConfig.count; i++) {
        mqttClient.subscribe(getRelayCommandTopic(i).c_str(), 1);
    }
    mqttClient.subscribe(getAllCommandTopic().c_str(), 1);
    
    // Publish online status
    publishAvailability(true);
    
    // Publish current state of all relays
...
void onMQTTDisconnect(AsyncMqttClientDisconnectReason reason) {
    ...
    if (mqttReconnectDelay < MQTT_RECONNECT_MAX) {
        mqttReconnectDelay *= MQTT_RECONNECT_MULTIPLIER;
    }
...
void onMQTTMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, 
    ...
    String prefix = String(mqttConfig.topic_prefix) + "/relay/";
    String allCommand = String(mqttConfig.topic_prefix) + "/relay/all/set";
    
    // Handle "all relays" command
    if (topicStr == allCommand) {
        handleMQTTAllCommand(payloadStr);
        return;
    }
    
    // Handle individual relay command
    if (topicStr.startsWith(prefix) && topicStr.endsWith("/set")) {
        String relayStr = topicStr.substring(prefix.length(), topicStr.length() - 4);
        int relay = relayStr.toInt();
        
        if (relay >= 0 && relay < gpioConfig.count) {
            handleMQTTCommand(relay, payloadStr);
        }
    }
}
...
void handleMQTTCommand(uint8_t relay, const char* payload) {
    ...
    if (strcasecmp(payload, "ON") == 0 || strcmp(payload, "1") == 0) {
        ...
        state = !lastRelayOutputs[relay];
    } else if (strcasecmp(payload, "AUTO") == 0) {
        // Return to schedule control
    ...
    // Publish state change acknowledgment
    publishRelayState(relay);
}
...
void handleMQTTAllCommand(const char* payload) {
    ...
    if (strcasecmp(payload, "ON") == 0 || strcmp(payload, "1") == 0) {
        ...
        return; // TOGGLE and AUTO not supported for all
    }
...
// =============================================================================
void publishRelayState(uint8_t relayIndex) {
    if (!mqttConnected || !mqttEnabled) return;
    if (relayIndex >= gpioConfig.count) return;
    
    String topic = getRelayStateTopic(relayIndex);
    bool state = lastRelayOutputs[relayIndex];
    bool manual = relayConfigs[relayIndex].manualOverride;
    
    String payload = state ? "ON" : "OFF";
    
    // Use retain flag so new subscribers get current state immediately
    mqttClient.publish(topic.c_str(), 1, true, payload.c_str());
    
    // Also publish manual/auto status
    String statusTopic = topic + "/mode";
    String statusPayload = manual ? "MANUAL" : "AUTO";
    mqttClient.publish(statusTopic.c_str(), 1, true, statusPayload.c_str());
    
    // Publish relay name
    String nameTopic = topic + "/name";
    mqttClient.publish(nameTopic.c_str(), 1, true, relayConfigs[relayIndex].name);
}
...
void publishAvailability(bool online) {
    ...
    if (!mqttConnected && online) return; // Can't publish online without connection
...
void handleMQTTStatusPublish() {
    ...
    unsigned long now = millis();
    if (timeHasElapsed(now, lastMQTTStatusPublish, MQTT_STATUS_INTERVAL)) {
        lastMQTTStatusPublish = now;
        publishAvailability(true);
        
        // Publish system metrics
        String prefix = String(mqttConfig.topic_prefix);
        mqttClient.publish((prefix + "/system/uptime").c_str(), 1, true, 
                          String(millis() / 1000UL).c_str());
        mqttClient.publish((prefix + "/system/freeheap").c_str(), 1, true, 
                          String(ESP.getFreeHeap()).c_str());
        mqttClient.publish((prefix + "/system/relay_count").c_str(), 1, true, 
                          String(gpioConfig.count).c_str());
    }
}
...
void initMQTT() {
    ...
    if (strlen(mqttConfig.username) > 0) {
        mqttClient.setCredentials(mqttConfig.username, mqttConfig.password);
    }
...
void connectToMQTT() {
    if (!mqttEnabled || !mqttInitialized) return;
    if (mqttClient.connected()) {
        if (!mqttConnected) mqttConnected = true;
        return;
    }
    if (!wifiConnected) return;
    
    unsigned long now = millis();
    if (timeHasElapsed(now, lastMQTTReconnectAttempt, mqttReconnectDelay)) {
...
void loadMQTTConfig() {
    ...
    if (len != sizeof(MQTTConfig) || mqttConfig.magic != MQTT_CFG_MAGIC) {
        // Initialize defaults
        memset(&mqttConfig, 0, sizeof(MQTTConfig));
        mqttConfig.magic = MQTT_CFG_MAGIC;
        mqttConfig.enabled = 0;
        mqttConfig.port = 1883;
        strcpy(mqttConfig.topic_prefix, "esp32_relay");
        saveMQTTConfig();
    }
}
...
bool SelfHealingSystem::recoverMQTT() {
    if (!mqttEnabled || !mqttInitialized) return false;
    
    unsigned long now = millis();
    if (!timeHasElapsed(now, lastMQTTReconfigure, 30000)) return false;
    
    lastMQTTReconfigure = now;
...
void SelfHealingSystem::smartRecovery() {
    ...
    // MQTT recovery
    if (mqttEnabled && !mqttConnected && wifiConnected) {
        if (timeHasElapsed(now, lastMQTTReconfigure, 60000)) {
            recoverMQTT();
            lastMQTTReconfigure = now;
        }
    }
...
// =============================================================================
//  SHARED CSS (unchanged from original)
// =============================================================================
...
// =============================================================================
//  HTML PAGES (index_html, wifi_html, ntp_html, ap_html, gpio_html, system_html unchanged from original, plus new mqtt_html)
// =============================================================================
...
// [wifi_html, ntp_html, ap_html, gpio_html, system_html are identical to original, just add MQTT link in nav]
...
<small>When disabled, ESP32 runs in AP-only mode (no internet/NTP/MQTT). Relay schedules using internal RTC/DS3231 will still work. You can still access this UI via the AP.</small>
</div>
<hr>
...
<button class="btn bscan" id="scanBtn" onclick="startScan()" style="white-space:nowrap">&#x1F4F6; Scan</button></div>
</div>
...
function loadWiFiStatus(){fetch('/api/wifi').then(r=>r.json()).then(d=>{document.getElementById('ssid').value=d.ssid||'';const staEnabled=d.sta_enabled!==undefined?d.sta_enabled:true;updateStaButton(staEnabled);const stationDiv=document.getElementById('stationConfig');if(stationDiv){stationDiv.style.opacity=staEnabled?'1':'0.5';const inputs=stationDiv.querySelectorAll('input, button');inputs.forEach(inp=>inp.disabled=!staEnabled)}const s=document.getElementById('status');s.style.display='';if(staEnabled&&d.connected){const bars=rssiBar(d.rssi||0);s.innerHTML='<span style="font-weight:700">\u2713 Connected</span> to: <strong>'+d.ssid+'</strong> ('+d.ip+') &nbsp;'+bars+' '+d.rssi+'dBm';s.className='alert ai'}else if(staEnabled&&!d.connected){s.innerHTML='<span style="font-weight:700">\u26A0 Not Connected</span><br>Configure SSID/password above to connect to a WiFi network.';s.className='alert aw'}else{s.innerHTML='<span style="font-weight:700">\u26D4 WiFi Station Disabled</span><br>ESP32 running in AP-only mode. Enable above to connect to WiFi networks for NTP/MQTT.';s.className='alert aw'}}).catch(()=>{})}
...
function toggleStation(){const btn=document.getElementById('staToggleBtn');const wasEnabled=btn.textContent==='\u2713 ON';const willEnable=!wasEnabled;btn.disabled=true;btn.textContent='...';fetch('/api/wifi',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sta_enabled:willEnable})}).then(r=>r.json()).then(d=>{btn.disabled=false;if(d.success){updateStaButton(willEnable);toast('WiFi Station '+(willEnable?'ON':'OFF'),true);if(!willEnable){toast('NTP/MQTT unavailable until re-enabled.',false)}const stationDiv=document.getElementById('stationConfig');if(stationDiv){stationDiv.style.opacity=willEnable?'1':'0.5';const inputs=stationDiv.querySelectorAll('input, button');inputs.forEach(inp=>inp.disabled=!willEnable)}loadWiFiStatus()}else{toast('Failed to toggle WiFi station',false);updateStaButton(wasEnabled)}}).catch(()=>{btn.disabled=false;toast('Error toggling WiFi station',false);updateStaButton(wasEnabled)})}
let scanTimer=null,scanning=false;
function startScan(){const staEnabled=document.getElementById('staToggleBtn').textContent==='\u2713 ON';if(!staEnabled){toast('Please enable WiFi Station Mode first',false);return}if(scanning)return;scanning=true;document.getElementById('scanBtn').textContent='\u23F3 Scanning\u2026';document.getElementById('scanBtn').disabled=true;const nl=document.getElementById('netlist');nl.style.display='block';nl.innerHTML='<div style="text-align:center;color:#90A4AE;padding:12px;font-size:13px">\uD83D\uDCE1 Scanning for networks...</div>';fetch('/api/wifi/scan',{method:'POST'}).then(()=>{scanTimer=setInterval(pollScan,2500)}).catch(()=>endScan())}
...
function renderNets(nets){const nl=document.getElementById('netlist');if(!nets.length){nl.innerHTML='<div style="color:#90A4AE;text-align:center;padding:8px;font-size:13px">No networks found.</div>';return}const frag=document.createDocumentFragment();const hdr=document.createElement('div');hdr.className='net-hdr';hdr.textContent='Available Networks';frag.appendChild(hdr);nets.sort((a,b)=>b.rssi-a.rssi).forEach(n=>{const d=document.createElement('div');d.className='netitem';const ns=document.createElement('span');ns.className='ns';ns.textContent=n.ssid;const nr=document.createElement('span');nr.className='nr';nr.textContent=n.rssi+'dBm';const bar=document.createElement('span');bar.innerHTML=rssiBar(n.rssi);const lock=document.createElement('span');lock.style.fontSize='13px';lock.textContent=n.enc?'\uD83D\uDD12':'';d.appendChild(ns);d.appendChild(nr);d.appendChild(bar);d.appendChild(lock);d.addEventListener('click',()=>{document.getElementById('ssid').value=n.ssid;document.getElementById('pw').focus()});frag.appendChild(d)});nl.innerHTML='';nl.appendChild(frag)}
function saveWiFi(){const staEnabled=document.getElementById('staToggleBtn').textContent==='\u2713 ON';if(!staEnabled){toast('Enable WiFi Station Mode first',false);return}const ssid=document.getElementById('ssid').value.trim();if(!ssid){toast('SSID required',false);return}const btn=document.querySelector('.bsave');btn.disabled=true;btn.textContent='Saving...';fetch('/api/wifi',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid,password:document.getElementById('pw').value})}).then(r=>r.json()).then(d=>{btn.disabled=false;btn.innerHTML='&#x1F4BE; Save &amp; Connect';if(d.success){toast('Credentials saved! Reconnecting...');setTimeout(()=>window.location.href='/',3000)}else{toast('Failed: '+d.error,false)}}).catch(()=>{btn.disabled=false;btn.innerHTML='&#x1F4BE; Save &amp; Connect';toast('Error',false)})}
loadWiFiStatus();
</script></body></html>)raw";

// ntp_html, ap_html, gpio_html, system_html are identical to original except nav includes MQTT link
// I'll include shortened versions that match the original structure
const char ntp_html[] PROGMEM = R"raw(<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Time</title><link rel="stylesheet" href="/style.css"></head><body>
<header><nav>
<a href="/">Relays</a><a href="/wifi">WiFi</a><a href="/ntp" class="cur">Time</a><a href="/ap">AP</a><a href="/gpio">GPIO</a><a href="/mqtt">MQTT</a><a href="/system">System</a>
</nav><div class="hdr-r"><span class="dot wd"></span><span class="dot td"></span>&nbsp;<span id="clk">--:--:--</span></div></header>
<main><p class="ptitle">Time &amp; NTP Settings</p><div class="card fcrd">
<div id="timeStatus" class="alert ai" style="display:none;margin-bottom:12px"></div>
<div class="fg"><label>NTP Server</label><input type="text" id="srv" placeholder="time.google.com" required><small>Fallbacks: time.google.com → time.windows.com → time.cloudflare.com → time.facebook.com</small></div>
...
<div class="fg"><label>Auto Sync (hours, 1-24)</label><input type="number" id="shi" min="1" max="24" value="1"></div>
...
function updateTimeStatus(d){const s=document.getElementById('timeStatus');s.style.display='block';let icon='',msg='',cls='ai';if(d.timeSource==='ntp'){icon='✅';msg='Time source: <strong>NTP Server</strong> — High accuracy';cls='ai'}else if(d.timeSource==='browser'){icon='🌐';msg='Time source: <strong>Browser Sync</strong> — Moderate accuracy';cls='aw'}else if(d.timeSource==='rtc'){icon='💿';msg='Time source: <strong>DS3231 Hardware RTC</strong> — Battery-backed';cls='ai'}else{icon='⚠️';msg='Time source: <strong>None</strong> — Please sync time';cls='aw'}let rtcInfo='';if(d.rtcPresent){rtcInfo='<br><small>✅ DS3231 RTC detected on GPIO21/22';if(d.rtcSynced)rtcInfo+=' | Last sync: '+d.rtcSyncAge+'s ago';rtcInfo+='</small>'}else{rtcInfo='<br><small>⚠️ DS3231 RTC not detected</small>'}s.innerHTML=icon+' '+msg+rtcInfo;s.className='alert '+cls}
setInterval(tick,1000);tick();
...
function save(){const h=parseInt(document.getElementById('shi').value);if(h<1||h>24){toast('Sync interval must be 1-24 h',false);return}fetch('/api/ntp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ntpServer:document.getElementById('srv').value,gmtOffset:parseInt(document.getElementById('gmt').value),daylightOffset:parseInt(document.getElementById('dst').value),syncHours:h})}).then(r=>r.json()).then(d=>{if(d.success)toast('NTP settings saved!');else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
function sync(){const b=document.getElementById('sbtn');b.disabled=true;b.textContent='Syncing…';fetch('/api/ntp/sync',{method:'POST'}).then(r=>r.json()).then(d=>{b.disabled=false;b.innerHTML='🔄 Sync Now';if(d.success)toast('Time synced via NTP!');else toast('Sync failed — check WiFi/NTP',false)}).catch(()=>{b.disabled=false;b.innerHTML='🔄 Sync Now';toast('Error',false)})}
function syncFromBrowser(){const b=document.getElementById('bsbtn');b.disabled=true;b.textContent='Syncing from browser…';const utcEpoch=Math.floor(Date.now()/1000);fetch('/api/time/browser-sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({utc_epoch:utcEpoch})}).then(r=>r.json()).then(d=>{b.disabled=false;b.innerHTML='🌐 Sync from Browser';if(d.success){document.getElementById('clk').textContent=d.local_time;let msg='Time synced from browser! Local time: '+d.local_time;if(d.rtc_synced)msg+=' | DS3231 updated';toast(msg);tick()}else toast('Sync failed: '+(d.error||'Unknown error'),false)}).catch(()=>{b.disabled=false;b.innerHTML='🌐 Sync from Browser';toast('Error syncing time from browser',false)})}
</script></body></html>)raw";

const char ap_html[] PROGMEM = R"raw(<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>AP</title><link rel="stylesheet" href="/style.css"></head><body>
<header><nav>
<a href="/">Relays</a><a href="/wifi">WiFi</a><a href="/ntp">Time</a><a href="/ap" class="cur">AP</a><a href="/gpio">GPIO</a><a href="/mqtt">MQTT</a><a href="/system">System</a>
</nav><div class="hdr-r"><span class="dot wd"></span><span class="dot td"></span>&nbsp;<span id="clk">--:--:--</span></div></header>
<main><p class="ptitle">Access Point Settings</p><div class="card fcrd">
...
function save(){const pw=document.getElementById('pw').value;if(pw.length>0&&pw.length<8){toast('Password must be 8+ chars or blank',false);return}fetch('/api/ap',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ap_ssid:document.getElementById('ssid').value,ap_password:pw,ap_channel:parseInt(document.getElementById('ch').value),ap_hidden:document.getElementById('hidden').value==='1'})}).then(r=>r.json()).then(d=>{if(d.success){toast('Settings saved!');if(d.restarted)toast('AP restarted - reconnect to new network');setTimeout(()=>location.reload(),4000)}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
</script></body></html>)raw";

const char gpio_html[] PROGMEM = R"raw(<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>GPIO</title><link rel="stylesheet" href="/style.css"></head><body>
<header><nav>
<a href="/">Relays</a><a href="/wifi">WiFi</a><a href="/ntp">Time</a><a href="/ap">AP</a><a href="/gpio" class="cur">GPIO</a><a href="/mqtt">MQTT</a><a href="/system">System</a>
</nav><div class="hdr-r"><span class="dot wd"></span><span class="dot td"></span>&nbsp;<span id="clk">--:--:--</span></div></header>
<main><p class="ptitle">GPIO Pin Configuration</p><div class="card fcrd">
<div class="alert ai">Configure which GPIO pins control your relays and set active level.</div>
<div class="fg"><label>Global Active Level Mode</label><select id="globalMode" onchange="saveGlobalMode()"><option value="0">Per-Relay Configuration</option><option value="1">Global: All Active LOW</option><option value="2">Global: All Active HIGH</option></select></div><hr>
<div class="fg"><label>Add New Relay Pin</label><div class="input-row"><select id="newPin"><option value="">Select GPIO...</option></select><button class="btn bsave" onclick="addPin()" style="margin-top:0;width:auto;padding:9px 20px">+ Add Relay</button></div></div><hr>
...
let gpioData = null;
function saveGlobalMode(){const mode=parseInt(document.getElementById('globalMode').value);fetch('/api/gpio/global-mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:mode})}).then(r=>r.json()).then(d=>{if(d.success){toast('Global active mode updated!');loadGPIO()}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
function loadGPIO(){fetch('/api/gpio').then(r=>r.json()).then(d=>{gpioData=d;document.getElementById('relayCount').textContent=d.count;fetch('/api/gpio/global-mode').then(r=>r.json()).then(data=>{if(data.mode!==undefined){document.getElementById('globalMode').value=data.mode}}).catch(()=>{});let pinsHtml='';const globalMode=parseInt(document.getElementById('globalMode').value);const isGlobalMode=(globalMode===1||globalMode===2);const globalModeText=(globalMode===1)?'GLOBAL ACTIVE LOW':(globalMode===2)?'GLOBAL ACTIVE HIGH':null;if(isGlobalMode){pinsHtml='<div style="background:#FFF3E0;padding:10px;border-radius:7px;margin-bottom:12px;border-left:4px solid #F9A825"><strong style="color:#E65100">⚠️ Global Mode Active:</strong> '+globalModeText+'<br><small style="color:#BF360C">Individual settings are overridden.</small></div>'}for(let i=0;i<d.count;i++){const activeLow=d.activeLow?d.activeLow[i]:true;pinsHtml+='<div style="display:flex;align-items:center;gap:10px;padding:10px;border:1px solid #E3E8EF;border-radius:7px;margin-bottom:6px;background:#FAFAFA"><span style="font-weight:700;min-width:60px">Relay '+(i+1)+'</span><span style="flex:1">GPIO <strong>'+d.pins[i]+'</strong></span><span style="flex:1;font-size:12px">Active: <strong>'+(activeLow?'LOW':'HIGH')+'</strong></span><button class="btn '+(activeLow?'bon-b':'boff-b')+'" onclick="toggleActiveLow('+i+')" style="padding:5px 10px;font-size:11px" '+ (isGlobalMode?'disabled':'')+'>'+(activeLow?'Set HIGH':'Set LOW')+'</button><button class="btn boff-b" onclick="deletePin('+i+')" style="padding:5px 10px;font-size:11px">🗑 Remove</button></div>'}if(d.count===0)pinsHtml+='<div style="color:#90A4AE;text-align:center;padding:20px">No relays configured.</div>';if(d.count>=16)pinsHtml+='<div style="color:#E65100;text-align:center;padding:10px;font-size:12px">Maximum 16 relays reached.</div>';document.getElementById('pins').innerHTML=pinsHtml;const select=document.getElementById('newPin');select.innerHTML='<option value="">Select GPIO...</option>';const usedPins=new Set(d.pins.slice(0,d.count));d.availablePins.forEach(pin=>{if(!usedPins.has(pin)){select.innerHTML+='<option value="'+pin+'">GPIO '+pin+'</option>'}});if(select.options.length===1){select.innerHTML+='<option value="" disabled>All available pins in use</option>'}}).catch(()=>toast('Error loading GPIO config',false))}
function toggleActiveLow(index){fetch('/api/gpio/toggle-active-low',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:index})}).then(r=>r.json()).then(d=>{if(d.success){toast('Active level: '+(d.activeLow?'LOW':'HIGH'));loadGPIO()}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
function addPin(){const pin=parseInt(document.getElementById('newPin').value);if(!pin)return;fetch('/api/gpio/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:pin})}).then(r=>r.json()).then(d=>{if(d.success){toast('Relay added on GPIO '+pin+'!');loadGPIO()}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
function deletePin(index){if(!confirm('Remove Relay '+(index+1)+'? All schedules will be lost.'))return;fetch('/api/gpio/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:index})}).then(r=>r.json()).then(d=>{if(d.success){toast('Relay removed!');loadGPIO()}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
function resetDefaults(){if(!confirm('Reset to default 16 GPIO pins?'))return;fetch('/api/gpio/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pins:DEFAULT_PINS})}).then(r=>r.json()).then(d=>{if(d.success){toast('Reset to default!');loadGPIO()}else toast('Failed: '+d.error,false)}).catch(()=>toast('Error',false))}
loadGPIO();
</script></body></html>)raw";

const char system_html[] PROGMEM = R"raw(<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>System</title><link rel="stylesheet" href="/style.css"></head><body>
<header><nav>
<a href="/">Relays</a><a href="/wifi">WiFi</a><a href="/ntp">Time</a><a href="/ap">AP</a><a href="/gpio">GPIO</a><a href="/mqtt">MQTT</a><a href="/system" class="cur">System</a>
</nav><div class="hdr-r"><span class="dot wd"></span><span class="dot td"></span>&nbsp;<span id="clk">--:--:--</span></div></header>
<main><p class="ptitle">System Information</p>
<div class="ibar" id="ibar">
<div class="ibox"><div class="l">STA IP</div><div class="v" id="sip">…</div></div>
<div class="ibox"><div class="l">AP IP</div><div class="v" id="sap">…</div></div>
<div class="ibox"><div class="l">Free Heap</div><div class="v" id="shp">…</div></div>
<div class="ibox"><div class="l">Uptime</div><div class="v" id="sup">…</div></div>
<div class="ibox"><div class="l">WiFi RSSI</div><div class="v" id="srs">…</div></div>
<div class="ibox"><div class="l">Time Source</div><div class="v" id="stsrc" style="font-size:13px">…</div></div>
<div class="ibox"><div class="l">UTC Epoch</div><div class="v" id="sutc" style="font-size:11px">…</div></div>
<div class="ibox"><div class="l">NTP Last Sync</div><div class="v" id="snt">…</div></div>
<div class="ibox"><div class="l">MQTT Status</div><div class="v" id="smqtt" style="font-size:12px">…</div></div>
<div class="ibox"><div class="l">Chip Model</div><div class="v" id="sch">…</div></div>
<div class="ibox"><div class="l">mDNS Hostname</div><div class="v" id="smdns">…</div></div>
<div class="ibox"><div class="l">Relay Count</div><div class="v" id="src">…</div></div>
<div class="ibox"><div class="l">GPIO Mode</div><div class="v" id="sactmode" style="font-size:12px">…</div></div>
<div class="ibox"><div class="l">DS3231 RTC</div><div class="v" id="srtc">…</div></div>
</div>
<div class="card fcrd"><p style="font-weight:700;margin-bottom:12px">Device Control</p>
<div style="display:flex;gap:8px;flex-wrap:wrap">
<button class="btn bwarn" onclick="rst()" style="padding:9px 18px">🔄 Verify Services</button>
<button class="btn bdanger" onclick="fct()" style="padding:9px 18px">⚠ Factory Reset</button>
</div>
<p style="color:#90A4AE;font-size:12px;margin-top:10px">Hold BOOT button for 5 seconds for hardware reset.</p>
</div></main><div id="toast"></div>
<script>
function toast(m,ok=true){const t=document.getElementById('toast');t.textContent=m;t.className='show '+(ok?'ok':'er');clearTimeout(t._t);t._t=setTimeout(()=>t.className='',3000);}
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='ntp')tc='g';else if(d.timeSource==='browser')tc='b';else if(d.timeSource==='rtc')tc='b';else tc='y';t.className='dot '+tc;}}).catch(()=>{});}
...
function loadSys(){fetch('/api/system').then(r=>r.json()).then(d=>{document.getElementById('sip').textContent=d.wifiConnected?d.ip:'(not connected)';document.getElementById('sap').textContent=d.ap_ip;document.getElementById('shp').textContent=(d.freeHeap/1024).toFixed(1)+' KB';document.getElementById('sup').textContent=fmtUp(d.uptime);document.getElementById('srs').textContent=d.wifiConnected?d.rssi+'dBm':'—';document.getElementById('stsrc').textContent=d.timeSource||'None';document.getElementById('sutc').textContent=d.utcEpoch||'—';document.getElementById('snt').textContent=d.ntpSyncAge>=0?fmtUp(d.ntpSyncAge):'Never';document.getElementById('smqtt').textContent=d.mqttConnected?'✅ Connected to '+d.mqttHost:'❌ Disconnected';document.getElementById('sch').textContent=d.chipModel||'ESP32-38P';document.getElementById('smdns').textContent=d.mdnsStarted?d.mdnsHostname+'.local':'Not running';document.getElementById('src').textContent=d.relayCount+' / '+d.maxRelays;let modeText='';if(d.globalActiveMode===1)modeText='🌍 Global LOW';else if(d.globalActiveMode===2)modeText='🌍 Global HIGH';else modeText='🔧 Per-Relay';document.getElementById('sactmode').textContent=modeText;document.getElementById('srtc').textContent=d.rtcPresent?'✅ Present':'❌ Not detected'}).catch(()=>{})}
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>MQTT</title><link rel="stylesheet" href="/style.css"></head><body>
<header><nav>
<a href="/">Relays</a><a href="/wifi">WiFi</a><a href="/ntp">Time</a><a href="/ap">AP</a><a href="/gpio">GPIO</a><a href="/mqtt" class="cur">MQTT</a><a href="/system">System</a>
</nav><div class="hdr-r"><span class="dot wd"></span><span class="dot td"></span>&nbsp;<span id="clk">--:--:--</span></div></header>
<main><p class="ptitle">MQTT Settings</p><div class="card fcrd">
<div id="status" class="alert ai" style="display:none"></div>
<div class="fg">
<label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;margin-bottom:8px">
...
<small>Enables remote control via MQTT protocol (Home Assistant, Node-RED, etc.). Requires WiFi Station to be connected.</small>
</div>
<hr>
<div id="mqttConfig">
<div class="fg"><label>MQTT Broker Host</label><input type="text" id="host" placeholder="192.168.1.100 or mqtt.local"></div>
<div class="fg"><label>Port</label><input type="number" id="port" value="1883" min="1" max="65535"></div>
<div class="fg"><label>Username (optional)</label><input type="text" id="user" placeholder="Leave blank if none"></div>
<div class="fg"><label>Password (optional)</label><input type="password" id="pass" placeholder="Leave blank if none"></div>
<div class="fg"><label>Topic Prefix</label><input type="text" id="prefix" value="esp32_relay"><small>Topics: {prefix}/relay/0, {prefix}/relay/0/set, {prefix}/status</small></div>
<button class="btn bsave" onclick="saveMQTT()">💾 Save MQTT Settings</button>
</div>
</div>
<div class="card fcrd" style="margin-top:16px">
<h3 style="margin-bottom:10px">MQTT Topic Reference</h3>
<div style="font-size:12px;font-family:monospace;background:#F5F5F5;padding:12px;border-radius:6px;line-height:1.8">
<strong>Publish (ESP32 → Broker):</strong><br>
<span id="tStatus">esp32_relay/status</span> → online/offline (retained)<br>
<span id="tState">esp32_relay/relay/0</span> → ON/OFF (retained)<br>
<span id="tMode">esp32_relay/relay/0/mode</span> → MANUAL/AUTO (retained)<br>
<span id="tName">esp32_relay/relay/0/name</span> → "Relay 1" (retained)<br><br>
<strong>Subscribe (Broker → ESP32):</strong><br>
<span id="tCmd">esp32_relay/relay/0/set</span> → ON, OFF, TOGGLE, AUTO<br>
<span id="tAll">esp32_relay/relay/all/set</span> → ON, OFF<br><br>
<strong>Home Assistant:</strong> Auto-discovery via MQTT
</div>
</div></main><div id="toast"></div>
<style>
.btn-mqtt-on{background:linear-gradient(135deg,#1565C0 0%,#0D47A1 100%);color:#fff;box-shadow:0 2px 4px rgba(21,101,192,0.3)}
.btn-mqtt-on:hover{background:linear-gradient(135deg,#1E88E5 0%,#1565C0 100%)}
.btn-mqtt-off{background:linear-gradient(135deg,#C62828 0%,#B71C1C 100%);color:#fff;box-shadow:0 2px 4px rgba(198,40,40,0.3)}
.btn-mqtt-off:hover{background:linear-gradient(135deg,#E53935 0%,#C62828 100%)}
</style>
<script>
function toast(m,ok=true){const t=document.getElementById('toast');t.textContent=m;t.className='show '+(ok?'ok':'er');clearTimeout(t._t);t._t=setTimeout(()=>t.className='',3000);}
function tick(){fetch('/api/time').then(r=>r.json()).then(d=>{document.getElementById('clk').textContent=d.time||'--:--:--';const w=document.querySelector('.wd'),t=document.querySelector('.td');if(w)w.className='dot '+(d.wifi?'g':'r');if(t){let tc='y';if(d.timeSource==='ntp')tc='g';else if(d.timeSource==='browser')tc='b';else if(d.timeSource==='rtc')tc='b';else tc='y';t.className='dot '+tc;}}).catch(()=>{});}
...
function updateTopicRefs(prefix){document.getElementById('tStatus').textContent=prefix+'/status';document.getElementById('tState').textContent=prefix+'/relay/0';document.getElementById('tMode').textContent=prefix+'/relay/0/mode';document.getElementById('tName').textContent=prefix+'/relay/0/name';document.getElementById('tCmd').textContent=prefix+'/relay/0/set';document.getElementById('tAll').textContent=prefix+'/relay/all/set'}
function loadMQTT(){fetch('/api/mqtt').then(r=>r.json()).then(d=>{document.getElementById('host').value=d.host||'';document.getElementById('port').value=d.port||1883;document.getElementById('user').value=d.username||'';document.getElementById('prefix').value=d.topic_prefix||'esp32_relay';updateMQTTButton(d.enabled);updateTopicRefs(d.topic_prefix||'esp32_relay');const mqttDiv=document.getElementById('mqttConfig');if(mqttDiv){mqttDiv.style.opacity=d.enabled?'1':'0.5';const inputs=mqttDiv.querySelectorAll('input, button');inputs.forEach(inp=>inp.disabled=!d.enabled)}const s=document.getElementById('status');s.style.display='';if(d.enabled&&d.connected){s.innerHTML='\u2705 <strong>Connected</strong> to '+d.host+':'+d.port;s.className='alert ai'}else if(d.enabled&&!d.connected){s.innerHTML='\u26A0 <strong>Enabled but not connected</strong>. Check broker address and WiFi.';s.className='alert aw'}else{s.innerHTML='\u26D4 <strong>MQTT Disabled</strong>. Enable above for remote control.';s.className='alert aw'}}).catch(()=>{})}
function toggleMQTT(){const btn=document.getElementById('mqttToggleBtn');const wasEnabled=btn.textContent==='\u2713 ON';const willEnable=!wasEnabled;btn.disabled=true;btn.textContent='...';fetch('/api/mqtt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:willEnable})}).then(r=>r.json()).then(d=>{btn.disabled=false;if(d.success){updateMQTTButton(willEnable);toast('MQTT '+(willEnable?'ON':'OFF'),true);const mqttDiv=document.getElementById('mqttConfig');if(mqttDiv){mqttDiv.style.opacity=willEnable?'1':'0.5';const inputs=mqttDiv.querySelectorAll('input, button');inputs.forEach(inp=>inp.disabled=!willEnable)}loadMQTT()}else{toast('Failed to toggle MQTT',false);updateMQTTButton(wasEnabled)}}).catch(()=>{btn.disabled=false;toast('Error',false);updateMQTTButton(wasEnabled)})}
function saveMQTT(){const btn=document.querySelector('.bsave');btn.disabled=true;btn.textContent='Saving...';const prefix=document.getElementById('prefix').value.trim()||'esp32_relay';fetch('/api/mqtt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:document.getElementById('mqttToggleBtn').textContent==='\u2713 ON',host:document.getElementById('host').value,port:parseInt(document.getElementById('port').value),username:document.getElementById('user').value,password:document.getElementById('pass').value,topic_prefix:prefix})}).then(r=>r.json()).then(d=>{btn.disabled=false;btn.innerHTML='\uD83D\uDCBE Save MQTT Settings';if(d.success){updateTopicRefs(prefix);toast('MQTT settings saved! Reconnecting...');setTimeout(()=>loadMQTT(),2000)}else toast('Failed: '+d.error,false)}).catch(()=>{btn.disabled=false;btn.innerHTML='\uD83D\uDCBE Save MQTT Settings';toast('Error',false)})}
document.getElementById('prefix').addEventListener('input',function(){updateTopicRefs(this.value||'esp32_relay')});
loadMQTT();
</script></body></html>)raw";

// =============================================================================
//  BOOT BUTTON FACTORY RESET (unchanged)
// =============================================================================
...
// =============================================================================
//  GPIO CONFIGURATION FUNCTIONS (unchanged)
// =============================================================================
void loadGPIOConfig() {
    preferences.begin(NVS_NAMESPACE, true);
...
// =============================================================================
//  SCHEDULE ENGINE (modified: added MQTT notification)
// =============================================================================
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        if (relayConfigs[i].manualOverride) {
            ...
            if (lastRelayOutputs[i] != targetState) {
                ...
                criticalStateDirty = true;
                // MQTT notification for manual state change
                if (mqttEnabled && mqttConnected) publishRelayState(i);
            }
            continue;
        }
        ...
        if (lastDebouncedState[i] != shouldBeOn) {
            if (timeHasElapsed(now, lastStateChange[i], 500)) {
                ...
                criticalStateDirty = true;
                // MQTT notification for schedule-driven state change
                if (mqttEnabled && mqttConnected) publishRelayState(i);
            }
        }
    }
...
// =============================================================================
//  SETUP (modified: added MQTT init)
// =============================================================================
void setup() {
    Serial.begin(115200);      
    initRTC();    
...
// =============================================================================
//  LOOP (modified: added MQTT handling)
// =============================================================================
void loop() {
    checkBootButton();    
    server.handleClient();
...
// =============================================================================
//  WEB SERVER SETUP (modified: added MQTT routes)
// =============================================================================
void setupWebServer() {
...
// =============================================================================
//  API HANDLERS (all original handlers unchanged, plus new MQTT handlers)
// =============================================================================
void handleGetRelays() {
    lastConnectionActivity = millis();    
...
void handleManualControl() {
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        ...
        criticalStateDirty = true;
        responseCache.valid = false;
        if (mqttEnabled && mqttConnected) publishRelayState(relay);
...
void handleGetMQTT() {
    ...
    char buffer[512];
    snprintf(buffer, sizeof(buffer),
        "{\"enabled\":%s,\"host\":\"%s\",\"port\":%d,\"username\":\"%s\","
        "\"topic_prefix\":\"%s\",\"connected\":%s}",
...
void handleSaveMQTT() {
    ...
    if (doc.containsKey("enabled")) {
        mqttConfig.enabled = doc["enabled"] ? 1 : 0;
    }
    
    const char* host = doc["host"];
    if (host) strncpy(mqttConfig.host, host, 63);
    ...
    if (username) strncpy(mqttConfig.username, username, 31);
    
    const char* password = doc["password"];
    if (password && strlen(password) > 0) strncpy(mqttConfig.password, password, 31);
    
    const char* prefix = doc["topic_prefix"];
    if (prefix && strlen(prefix) > 0) strncpy(mqttConfig.topic_prefix, prefix, 31);
    ...
    if (mqttConfig.enabled && !wasEnabled) {
        ...
        mqttEnabled = true;
        initMQTT();
    } else if (!mqttConfig.enabled && wasEnabled) {
        // Disabling MQTT
        publishAvailability(false);
        mqttClient.disconnect(true);
        mqttEnabled = false;
...
// =============================================================================
//  GPIO MANAGEMENT HANDLERS (unchanged)
// =============================================================================
void handleGetGPIOConfig() {
    lastConnectionActivity = millis();    
    DynamicJsonDocument doc(2048);
