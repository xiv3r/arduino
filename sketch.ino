...
struct TimerSchedule {
    ...
    uint16_t months[8];     // Month of year (bitmask: Jan=bit0, Feb=bit1, etc.)
};
...
static int      cachedMonth    = 0;  // 0-11 (January = 0)
...
// =============================================================================
//  DS3231 RTC FUNCTIONS - FIXED VERSION
// =============================================================================
void initRTC() {
    Wire.begin(21, 22);
    Wire.setTimeOut(50);  // FIX: 50ms timeout prevents hanging when DS3231 absent
...
void immediateDS3231Sync() {
    // FIX: Guard against calling when RTC is not present or not initialized
    if (!rtcPresent || !rtcInitialized || internalEpoch <= 0) return;
    
    // FIX: Remove redundant rtc.begin() call - already initialized in initRTC()
...
void syncInternalRTCFromDS3231() {
    // FIX: Add rtcTimeValid check to prevent accessing RTC with invalid time
...
time_t getCurrentEpoch() {
    ...
    // FIX: Add rtcTimeValid check to prevent hanging on absent DS3231
...
// FIX: Complete rewrite of recoverRTC to avoid hanging
bool SelfHealingSystem::recoverRTC() {
    if (!rtcPresent) return false;
    
    // FIX: Do NOT call Wire.begin() again - already initialized in initRTC()
    // FIX: Do NOT call rtc.begin() again - already initialized in initRTC()
    ...
    // If we get here, RTC data is invalid
    rtcTimeValid = false;
    return false;
}
...
void SelfHealingSystem::performTargetedRecovery() {
    ...
    liveReconfigureAP();
    delay(50);
    syncInternalRTCFromDS3231();  // Safe now - has proper guards
    verifyRelayStates();    
...
// =============================================================================
//  BROWSER TIME SYNC - FIXED VERSION
// =============================================================================
void handleBrowserTimeSync() {
    ...
    // FIX: Guard RTC sync with proper check - no rtc.begin() call
...
void updateScheduleCache() {
    ...
    cachedMonth    = ti->tm_mon;  // 0-11 (January = 0)
...
void setup() {
    ...
    for (int i = 0; i < MAX_RELAYS; i++) {
        for (int s = 0; s < 8; s++) {
            ...
            relayConfigs[i].schedule.months[s] = MONTH_ALL;  // All months enabled by default
        }
        relayConfigs[i].manualOverride = false;
        relayConfigs[i].manualState    = false;
