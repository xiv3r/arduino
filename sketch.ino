...
// =============================================================================
//  Schedule Engine (FIXED)
// =============================================================================
static uint8_t  cachedTodayBit = DAY_ALL;  // FIX: Initialize to DAY_ALL instead of 0
...
// =============================================================================
//  Debounce State (FIX: Moved to global scope)
// =============================================================================
...
void updateNTPSync();
void syncDebounceStates();  // FIX: New function declaration
...
// =============================================================================
//  INDEX PAGES (ALL HTML UNCHANGED - COMPLETE)
// =============================================================================
...
// =============================================================================
//  FIX: Sync Debounce States with Schedule Cache
// =============================================================================
void syncDebounceStates() {
    for (int i = 0; i < gpioConfig.count; i++) {
        if (!relayConfigs[i].manualOverride) {
            lastDebouncedState[i] = scheduleActiveCache[i];
            lastStateChange[i] = 0;
        }
    }
}
...
// =============================================================================
//  FIX: Corrected SCHEDULE CACHE Function
// =============================================================================
void updateScheduleCache() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            // FIX: Correct month day semantics
            bool monthDayMatches = true;
            if (monthDayMask != 0) {
                monthDayMatches = (monthDayMask & (1 << (currentMonthDay - 1))) != 0;
            }
            bool monthMatches = true;
            if (monthMask != 0) {
                monthMatches = (monthMask & (1 << currentMonth)) != 0;
            }
            bool dayMatches = (relayConfigs[i].schedule.days[s] & cachedTodayBit) != 0;
            
            if (!monthDayMatches || !monthMatches || !dayMatches) continue;
            ...
            if (start == stop) {
                ...
                if (cur >= start) {
                    ...
                    if (checkDate) {
                        bool yesterdayDayMatches = (relayConfigs[i].schedule.days[s] & (1 << checkDate->tm_wday)) != 0;
                        bool yesterdayMonthDayMatches = true;
                        if (monthDayMask != 0) {
                            yesterdayMonthDayMatches = (monthDayMask & (1 << (checkDate->tm_mday - 1))) != 0;
                        }
                        bool yesterdayMonthMatches = true;
                        if (monthMask != 0) {
                            yesterdayMonthMatches = (monthMask & (1 << checkDate->tm_mon)) != 0;
                        }
                        
                        if (yesterdayDayMatches && yesterdayMonthDayMatches && yesterdayMonthMatches) {
                            hasActive = true;
                            break;
                        }
                    }
...
void processRelaySchedules() {
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        // FIX: Use cached state directly
        bool shouldBeOn = scheduleActiveCache[i];
...
void setup() {
    ...
    setupWebServer();
    updateScheduleCache();
    syncDebounceStates();  // FIX: Sync debounce states after cache update
...
// FIX: handleManualControl now syncs debounce state
void handleManualControl() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        ...
        lastDebouncedState[relay] = state;  // FIX: Sync debounce state
        lastStateChange[relay] = 0;          // FIX: Reset state change timer
        saveConfiguration();
        criticalStateDirty = true;
        responseCache.valid = false;
        server.send(200, "application/json", "{\"success\":true}");
...
// FIX: handleResetManual now resets debounce and applies state immediately
void handleResetManual() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        ...
        lastScheduleCacheUpdate = 0;  // FIX: Force cache refresh
        updateScheduleCache();
        saveConfiguration();
        
        // FIX: Reset debounce and immediately apply
        lastDebouncedState[relay] = scheduleActiveCache[relay];
        lastStateChange[relay] = 0;
        setRelayOutput(relay, scheduleActiveCache[relay]);
        lastRelayOutputs[relay] = scheduleActiveCache[relay];
...
// FIX: handleSaveRelay now forces immediate refresh and applies state
void handleSaveRelay() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
    ...
    for (JsonObject sch : schedules) {
        ...
        // FIX: Keep 0 as "no restriction" instead of converting to MONTH_ALL
        relayConfigs[relay].schedule.monthMask[s] = rawMonthMask;
        s++;
    }    
    saveConfiguration();
    lastScheduleCacheUpdate = 0;  // FIX: Force cache refresh
...
// FIX: handleSaveGPIOConfig now forces cache refresh
void handleSaveGPIOConfig() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
    ...
    lastScheduleCacheUpdate = 0;  // FIX: Force cache refresh
    updateScheduleCache();
    syncDebounceStates();  // FIX: Sync debounce after config change
    criticalStateDirty = true;
    responseCache.valid = false;    
    server.send(200, "application/json", "{\"success\":true,\"count\":" + String(newCount) + "}");
}

// FIX: handleAddGPIO now forces cache refresh
void handleAddGPIO() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
...
// FIX: handleDeleteGPIO now forces cache refresh
void handleDeleteGPIO() {
    lastConnectionActivity = millis();    
    if (!server.hasArg("plain")) {
        server.send(400, "application/json", "{\"success\":false,\"error\":\"No data\"}"); 
