...
void updateScheduleCache() {
    ...
    uint64_t yesterdayEpoch = epoch - 86400ULL;
    uint64_t yesterdayLocalEpoch = getLocalEpoch(yesterdayEpoch);
    struct tm* yesterday = gmtime64(&yesterdayLocalEpoch);    
    int yesterdayWeekday = yesterday ? yesterday->tm_wday : currentWeekday;
    int yesterdayMonthDay = yesterday ? yesterday->tm_mday : currentMonthDay;
    int yesterdayMonth = yesterday ? yesterday->tm_mon : currentMonth;    
    ...
    for (int i = 0; i < gpioConfig.count; i++) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            auto matchesDayMonth = [&](int weekday, int monthDay, int month) -> bool {
                if (dayMask != 0x7F && !(dayMask & (1 << weekday))) {
                    return false;
                }
                if (monthMask != 0 && monthMask != MONTH_ALL) {
                    if (!(monthMask & (1 << month))) return false;
                }
                if (monthDayMask != 0 && monthDayMask != 0xFFFFFFFF) {
                    if (!(monthDayMask & (1 << (monthDay - 1)))) return false;
                }
                return true;
            };            
            if (start == stop) {
                if (matchesDayMonth(currentWeekday, currentMonthDay, currentMonth)) {
                    hasActive = true;
                    break;
                }
            } else if (start < stop) {
                ...
                } else if (currentSeconds < stop) {
                    if (matchesDayMonth(yesterdayWeekday, yesterdayMonthDay, yesterdayMonth)) {
                        hasActive = true;
                        break;
                    }
                }
