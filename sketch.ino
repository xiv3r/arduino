...
// DS3231 temperature cache (I2C throttled to once per 10 s)
...
struct SystemConfig {
    ...
    char     ap_ssid[32];
    char     ap_password[32];
    char     ntp_server[48];       // kept for NVS backward compatibility
    int32_t  gmt_offset;
...
struct ExtConfig {
    uint8_t magic;
    uint8_t ap_channel;
    uint8_t ntp_sync_hours;        // kept for NVS backward compatibility
    uint8_t ap_hidden;
...
// =============================================================================
//  DS3231 Temperature (cached, throttled to once per 10 s)
//  Registers: 0x11 = signed integer degC, 0x12 upper 2 bits = 0.25 degC steps
// =============================================================================
float getRTCTemperature() {
    ...
    Wire.write(0x11);
    if (Wire.endTransmission() != 0) return cachedRtcTemp;      // keep previous value

    if (Wire.requestFrom((uint8_t)0x68, (uint8_t)2) != 2) {
        return cachedRtcTemp;
    }

    int8_t  msb = (int8_t)Wire.read();     // signed integer part
    uint8_t lsb = Wire.read();              // upper 2 bits = fractional
...
// =============================================================================
//  SYNC INTERNAL RTC (browser-synced source)
// =============================================================================
void syncInternalRTC(uint64_t rawUtcEpoch) {
...
void initDefaults() {
    ...
    strcpy(sysConfig.ntp_server,  "");   // kept for NVS backward compatibility
...
void loadExtConfig() {
    ...
    if (len != sizeof(ExtConfig) || extConfig.magic != EXT_CFG_MAGIC) {
        ...
        extConfig.ap_channel     = 6;
        extConfig.ntp_sync_hours = 1;   // kept for NVS backward compatibility
