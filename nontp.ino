 ...
 *  License: GPLv3
 *
 *  v3.2 — RTClib removed. Custom DS3231 I2C driver using native uint64_t epoch.
 *  v3.3 — Compilation fix: checksum helper moved into SelfHealingSystem class
 *         to prevent Arduino auto-prototype ordering error.
===============================================================================
*/

/*
===============================================================================
 *  v3.1 LONGEVITY PATCH SET (every change marked "v3.1 FIX" in source)
 *  1. NVS flash wear: epoch-only RTC saves (v2 rewrote the full ~2.4KB config
 *     blob every hour, ~22MB/year -> hottest sectors die in ~2-6 years);
 *     manual relay states now persist via a rate-limited small critical-state
 *     blob instead of per-click full config writes.
 *  2. Critical state is now actually RESTORED on boot (v2 wrote the blob but
 *     never read it back).
 *  3. millis() rollover: 64-bit wrap-safe uptime accumulator (was 49.7-day
 *     wrap in the system UI).
 *  4. MAX_UNIX_TIME_64 capped at exact 2500-01-01T00:00:00Z (was 2^64-1,
 *     which let garbage epochs pass every sanity check).
 *  5. mDNS: removed periodic service re-registration (duplicate-record churn
 *     and slow heap growth on some arduino-esp32 core versions).
 *  6. Misc: fresh weekday bit in schedule engine, duplicate work removed from
 *     browser time sync, stray NVS end() removed.
 *  Note: this is the no-NTP, no-MQTT build - DS3231 (or browser sync) is the
 *  time authority. sysConfig.ntp_server / extConfig.ntp_sync_hours are
 *  retained only for NVS layout compatibility and are unused.
 *  Everything not listed above is byte-identical to the original source.
===============================================================================
*/
...
#include <ESPmDNS.h>
#include <Wire.h>
// RTClib removed — DS3231 accessed directly via Wire
...
//  Year 2106+ Support
// =============================================================================
// v3.1 FIX: sane upper bound - exact 2500-01-01T00:00:00Z. The old 2^64-1
//     limit let wrapped or garbage epochs (e.g. NTP-era misreads past 2095)
//     pass every sanity check and pin the schedule engine to a bogus date.
...
// =============================================================================
//  DS3231 Direct I2C Driver  (replaces RTClib)
// =============================================================================
...
// Days from civil date (Howard Hinnant's algorithm) — matches gmtime64
...
// Low-level I2C
bool ds3231ReadRaw(uint8_t reg, uint8_t *buf, uint8_t len) {
    Wire.beginTransmission(DS3231_ADDR);
    Wire.write(reg);
    if (Wire.endTransmission(false) != 0) return false;
    if (Wire.requestFrom((uint8_t)DS3231_ADDR, len) != len) return false;
    for (uint8_t i = 0; i < len; i++) buf[i] = Wire.read();
    return true;
}

bool ds3231WriteRaw(uint8_t reg, const uint8_t *buf, uint8_t len) {
    Wire.beginTransmission(DS3231_ADDR);
    Wire.write(reg);
...
bool ds3231LostPower() {
    uint8_t status = 0;
    if (!ds3231ReadRaw(DS3231_REG_STATUS, &status, 1)) return true;
    return (status & 0x80) != 0;   // OSF bit
}

// Read epoch (UTC) from DS3231 (uint64_t)
bool ds3231ReadEpoch(uint64_t &outEpoch) {
    uint8_t status = 0;
    if (!ds3231ReadRaw(DS3231_REG_STATUS, &status, 1)) return false;
    if (status & 0x80) return false;   // OSF → time not trustworthy

    uint8_t buf[7];
    if (!ds3231ReadRaw(DS3231_REG_SEC, buf, 7)) return false;
    ...
    if (buf[2] & 0x40) {                          // 12h mode
        ...
        if (buf[2] & 0x20) hour += 12;            // PM
        if (hour == 12 && !(buf[2] & 0x20)) hour = 0;  // 12 AM
    } else {                                       // 24h mode
        hour = bcd2bin(buf[2] & 0x3F);
    }
    uint8_t date  = bcd2bin(buf[4] & 0x3F);
    uint8_t month = bcd2bin(buf[5] & 0x1F);
    uint16_t year = 2000 + bcd2bin(buf[6]);
    ...
    outEpoch = (uint64_t)(days * 86400LL + hour * 3600 + min * 60 + sec);
    return true;
}

// Write epoch (UTC) to DS3231
bool ds3231WriteEpoch(uint64_t epoch) {
    if (epoch < MIN_UNIX_TIME_64 || epoch >= MAX_UNIX_TIME_64) return false;

    uint64_t days = epoch / 86400ULL;
    uint32_t rem  = (uint32_t)(epoch % 86400ULL);
    uint8_t hour  = rem / 3600;
    uint8_t min   = (rem % 3600) / 60;
    uint8_t sec   = rem % 60;
    ...
    buf[0] = bin2bcd(sec);
    buf[1] = bin2bcd(min);
    buf[2] = bin2bcd(hour);                       // force 24h mode
    buf[3] = (uint8_t)(((days + 4) % 7) + 1);     // DOW 1-7 (1=Sunday)
    buf[4] = bin2bcd((uint8_t)d);
    buf[5] = bin2bcd((uint8_t)m);                 // century = 0 (2000-2099)
    buf[6] = bin2bcd((uint8_t)(y - 2000));

    if (!ds3231WriteRaw(DS3231_REG_SEC, buf, 7)) return false;

    // Clear OSF
    uint8_t status = 0;
    if (ds3231ReadRaw(DS3231_REG_STATUS, &status, 1)) {
        status &= ~0x80;
        ds3231WriteRaw(DS3231_REG_STATUS, &status, 1);
    }
    return true;
}
...
// =============================================================================
//  Self-Healing Class
//  NOTE: calculateChecksumOf() is a static member so Arduino's auto-prototype
//        generator does NOT emit a free-function prototype referencing
//        CriticalRelayState above its definition.
// =============================================================================
...
public:
    ...
    // Checksum helper — member function (not free) so Arduino auto-prototype
    // generator leaves it alone.
    static uint32_t calculateChecksumOf(const CriticalRelayState& st);
...
//  Inline Helper
// =============================================================================
// v3.1 FIX: rollover-safe uptime (millis() wraps every 49.7 days)
...
// =============================================================================
//  DS3231 RTC Functions (RTClib-free)
// =============================================================================
void initRTC() {
    rtcPresent = false;
    rtcTimeValid = false;
    ...
    uint64_t rtcEpoch = 0;
    if (!ds3231ReadEpoch(rtcEpoch)) return;
    if (!VALID_UNIX_TIME_64(rtcEpoch)) return;
...
void immediateDS3231Sync() {
    if (rtcPresent && rtcInitialized && internalEpoch > 0) {
        if (ds3231WriteEpoch(internalEpoch)) {
            rtcTimeValid = true;
            lastRTCDSync = millis();
        }
    }
...
float getRTCTemperature() {
    ...
    uint8_t raw[2];
    if (!ds3231ReadRaw(DS3231_REG_TEMP, raw, 2)) return cachedRtcTemp;
...
uint64_t getCurrentEpoch() {
    ...
    if (rtcPresent && rtcTimeValid) {
        uint64_t rtcEpoch = 0;
        if (ds3231ReadEpoch(rtcEpoch) && VALID_UNIX_TIME_64(rtcEpoch)) {
            return rtcEpoch;
        }
    }
...
void saveRTCState() {
    // v3.1 FIX: persist ONLY the epoch as an 8-byte NVS primitive. v2 rewrote
    //     the full ~2.4 KB config blob (sysConfig + relayConfigs) on every
    //     NTP/browser sync AND once per hour: ~22 MB/year of flash traffic ->
    //     hottest NVS sectors accumulate ~44k erases/decade -> sector death in
    //     ~2-6 years on 10k-cycle flash. This cut reduces entry volume ~50x.
    performRTCReabase();
...
void loadRTCState() {
    // v3.1 FIX: read the dedicated rtcEpoch key; fall back to the legacy
    //     sysConfig.last_rtc_epoch field so existing installs migrate silently.
...
void autoSaveInternalRTC() {
    ...
    if (timeHasElapsed(now, lastInternalRTCSave, INTERNAL_RTC_SAVE_INTERVAL)) {
        lastInternalRTCSave = now;
        saveRTCState();   // v3.1 FIX: epoch-only save (was full saveConfiguration)
    }
}
...
bool loadRTCFromDS3231() {
    ...
    uint64_t rtcEpoch = 0;
    if (!ds3231ReadEpoch(rtcEpoch)) return false;
    if (!VALID_UNIX_TIME_64(rtcEpoch)) return false;
...
//  NOTE: calculateChecksumOf is defined as a member function of
//        SelfHealingSystem to avoid Arduino's auto-prototype ordering error.
//        The free helper calculateCriticalChecksum() forwards to it — it has
//        no custom-type parameters, so its auto-prototype is harmless.
// =============================================================================
uint32_t SelfHealingSystem::calculateChecksumOf(const CriticalRelayState& st) {
    uint32_t sum = 0;
    for (int i = 0; i < MAX_RELAYS; i++) {
        sum += st.relayStates[i] ? (1u << (i % 32)) : 0u;
        sum += st.manualOverrides[i] ? (1u << ((i + 16) % 32)) : 0u;
    }
    return sum ^ st.timestamp;
}
uint32_t calculateCriticalChecksum() {
    // v3.1 FIX: factored so restoreCriticalState() can validate a loaded blob
    //     with the exact same checksum function that saved it.
    return SelfHealingSystem::calculateChecksumOf(criticalState);
}
...
void SelfHealingSystem::saveCriticalState() {
    ...
    criticalState.checksum = calculateChecksumOf(criticalState);
...
bool SelfHealingSystem::restoreCriticalState() {
    // v3.1 FIX: actually RESTORE the critical-state blob that saveCriticalState()
    //     has been writing all along. In v2 the blob was write-only (never read
    //     back), so manual overrides set via single-relay MQTT commands were
    //     silently lost on reboot while web-UI toggles were not. Now every
    //     manual change (web UI, MQTT single, MQTT all) persists through the
    //     rate-limited critical save (see loop()) and is restored here.
    CriticalRelayState saved = {};   // value-init: zeroed, then NSDMI magic
    preferences.begin(NVS_NAMESPACE, true);
    size_t len = preferences.getBytes("criticalState", &saved, sizeof(saved));
    preferences.end();
    bool blobValid = (len == sizeof(saved))
                     && (saved.magic == 0xDEADBEEF)
                     && (saved.checksum == calculateChecksumOf(saved));
    if (blobValid) {
    ...
    // Legacy path (no blob / invalid blob): pre-v3.1 behaviour - restore
    // whatever manual overrides were last written into the relayConfigs blob.
    for (int i = 0; i < gpioConfig.count; i++) {
...
void SelfHealingSystem::smartRecovery() {
    ...
    // v3.1 FIX: removed the periodic MDNS.addService() re-registration blocks.
    //     Re-adding an existing service every 5 minutes is wasted work and on
    //     some arduino-esp32 core versions appends duplicate service records
    //     (unbounded slow heap growth over years). ESP-IDF mDNS re-announces
    //     on its own TTL schedule; no manual re-add is needed.
...
void performMemoryCleanup() {
    ...
    // v3.1 FIX: removed stray preferences.end() - no NVS session is open here
    //     (every save/load helper opens and closes its own); ending an
    //     unowned session is at best a no-op and at worst unbalances NVS.
    delay(50);
    for (int i = 0; i < 5; i++) {
        void* ptr = malloc(512);
        if (ptr) free(ptr);
...
void handleBrowserTimeSync() {
    ...
    // v3.1 FIX: removed the duplicate DS3231 adjust + saveRTCState +
    //     updateScheduleCache block - syncInternalRTC() already did all three.
    criticalStateDirty = true;
...
void processRelaySchedules() {
    ...
    int currentSeconds = ti->tm_hour * 3600 + ti->tm_min * 60 + ti->tm_sec;
    int cur = currentSeconds;
    uint8_t todayBit = (1 << currentWeekday); // v3.1 FIX: fresh bit (cached value could lag 1 s across midnight)
...
void loop() {
    ...
    unsigned long now = millis();
    updateUptime();  // v3.1 FIX: rollover-safe uptime accumulator
    if (server.client()) {
        lastConnectionActivity = now;
    }
    ...
    if (rtcPresent && rtcTimeValid && rtcInitialized) {
        if (timeHasElapsed(now, lastRTCDSync, DS3231_SYNC_INTERVAL)) {
            lastRTCDSync = now;
            uint64_t dsEpoch = 0;
            if (ds3231ReadEpoch(dsEpoch) && VALID_UNIX_TIME_64(dsEpoch)) {
    ...
    // v3.1 FIX: flush dirty critical state within 60 s (was 2 h). An isolated
    //     manual change (last flush >60 s ago) persists on the very next loop
    //     pass; bursts are debounced to at most one ~44-byte blob write per
    //     minute. Worst case (an automation toggling relays once per second,
    //     24/7) is ~1,440 small writes/day - about 100x less flash traffic
    //     than v2's full ~2.4 KB blob write per command; realistic usage
    //     (hundreds of changes/day) leaves decades of flash headroom.
...
void handleManualControl() {
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        ...
        // v3.1 FIX: no immediate full-blob saveConfiguration() per click -
        //     the manual state persists via the rate-limited critical save in
        //     loop() (isolated changes flush on the next loop pass) and is
        //     restored on boot by restoreCriticalState().
        criticalStateDirty = true;
        responseCache.valid = false;
        server.send(200, "application/json", "{\"success\":true}");
    } else {
...
void handleResetManual() {
    ...
    if (relay >= 0 && relay < gpioConfig.count) {
        ...
        updateScheduleCache();
        // v3.1 FIX: deferred critical save instead of full-blob write per click
        //     (persisted within 60 s by the loop() critical flush)
        criticalStateDirty = true;
        responseCache.valid = false;
        server.send(200, "application/json", "{\"success\":true}");
    } else {
...
void handleGetSystem() {
    ...
    doc["uptime"] = getUptimeSeconds();  // v3.1 FIX: no 49.7-day wrap in UI
